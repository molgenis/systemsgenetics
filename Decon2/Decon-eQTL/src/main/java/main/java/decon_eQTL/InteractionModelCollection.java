...
public class InteractionModelCollection {
	...
	public void findBestFullModel() throws IllegalAccessException, IOException {
		...
		for (String modelName : getFullModelNames()) {
			...
			int nrSamplesWithGenotypes = 0;
			for (int d = 0; d < genotypes.length; d++) {
				if (genotypes[d] != -1) {
					nrSamplesWithGenotypes++;
				}
			}

			double[] actualExpression = new double[nrSamplesWithGenotypes];
			int sctr = 0;
			for (int d = 0; d < genotypes.length; d++) {
				if (genotypes[d] != -1) {
					actualExpression[sctr] = expressionValues[d];
					sctr++;
				}
			}

			if (useOLS) {
				fullModel.calculateSumOfSquaresOLS(actualExpression);
			} else {
				fullModel.calculateSumOfSquaresNNLS(actualExpression);
			}
	...
	public void findBestCtModel() throws IllegalAccessException, IOException {
		...
		for (String celltype : celltypes) {
			...
			for (String modelName : getCtModelNames(celltype)) {
				...
				if (useOLS) {
					ctModel.calculateSumOfSquaresOLS(actualExpression);
				} else {
					ctModel.calculateSumOfSquaresNNLS(actualExpression);
				}
	...
	public void createObservedValueMatricesFullModel(Boolean addGenotypeTerm)
		...
		for (String genotypeConfiguration : getGenotypeConfigurationsFullModel()) {
			...
			// determine how many samples actually have genotypes
			int nrNonMissingGenotypes = 0;
			for (int sampleIndex = 0; sampleIndex <= numberOfSamples - 1; ++sampleIndex) {
				if (genotypes[sampleIndex] != -1) {
					// -1 is code for missing genotype
					nrNonMissingGenotypes++;
				}
			}
			InteractionModel fullModel = new InteractionModel(nrNonMissingGenotypes,
					numberOfTerms);
			if (addGenotypeTerm) {
				// add one to number of terms because we are adding the genotype as a separate term
				fullModel = new InteractionModel(nrNonMissingGenotypes, numberOfTerms + 1);
			}
			...
			for (int sampleIndex = 0; sampleIndex <= numberOfSamples - 1; ++sampleIndex) {
				if (genotypes[sampleIndex] != -1) {
					...
					for (int celltypeIndex = 0; celltypeIndex < numberOfCelltypes; ++celltypeIndex) {
						...
						fullModel.addObservedValue(celltypePerc, sampleCtr, celltypeIndex);
						...
						fullModel.addObservedValue(celltypePerc * genotypes[sampleIndex],
								sampleCtr, numberOfCelltypes + celltypeIndex);
	...
	public void createObservedValueMatricesCtModels(Boolean addGenotypeTerm)
		...
		for (String genotypeConfiguration : getGenotypeConfigurationsCtModel()) {
			...
			for (int modelIndex = 0; modelIndex < numberOfCelltypes; modelIndex++) {
				...
				InteractionModel ctModel = new InteractionModel(nrSamplesWithGenotypes, numberOfTerms);
				if (addGenotypeTerm) {
					// add one to number of terms because we are adding the genotype as a separate term
					ctModel = new InteractionModel(nrSamplesWithGenotypes, numberOfTerms + 1);
				}
				...
				for (int sampleIndex = 0; sampleIndex <= numberOfSamples - 1; sampleIndex++) {
					...
					if (genotypes[sampleIndex] != -1) {
						...
						for (int celltypeIndex = 0; celltypeIndex < numberOfCelltypes; celltypeIndex++) {
							...
							ctModel.addObservedValue(celltype_perc, sampleCtr, celltypeIndex);
							if (sampleIndex == 0) {
							...
							if (celltypeIndex != modelIndex) {
								...
								ctModel.addObservedValue(celltype_perc * genotype, sampleCtr, genotypeCounter);
						...
						if (addGenotypeTerm) {
							...
							ctModel.addObservedValue(genotypes[sampleIndex],
									sampleCtr, (numberOfCelltypes * 2) - 1);
						}
