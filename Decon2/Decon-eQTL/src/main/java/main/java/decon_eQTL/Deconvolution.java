...
public class Deconvolution {
	 ...
	 *
	 * @param deconvolutionResults The deconvolution result
	 */
...
//		double[][] cellcts = cellCounts.getCellCountPercentages(); // [samples][cellcts]
//		double[] actualExpression = new double[nrSamplesWithGenotypes];
//		double[] actualGenotypes = new double[nrSamplesWithGenotypes];
//		int sampleCtr = 0;
//		for (int i = 0; i < genotypes.length; ++i) {
//			if (genotypes[i] != -1) {
//				actualExpression[sampleCtr] = expression[i];
//				actualGenotypes[sampleCtr] = genotypes[i];
//				sampleCtr++;
//			}
//		}
		...
		if (commandLineOptions.getWholeBloodQTL()) {
			...
			wholeBloodQTLpvalue = Statistics.calculateSpearmanTwoTailedPvalue(wholeBloodQTL, nrSamplesWithGenotypes);
		}
	...
	private void calculateDeconvolutionPvalue(InteractionModelCollection interactionModelCollection)
		...
		for (int modelIndex = 0; modelIndex < cellCounts.getNumberOfCelltypes(); ++modelIndex) {
			...
			double pval = Statistics.anova(fullModel.getSumOfSquares(), ctModel.getSumOfSquares(),
					...
					true);
//          debug:
//			System.out.println(celltypeName + "\tFullModelRSS: " + fullModel.getSumOfSquares() + "\tCtModelRSS: " + ctModel.getSumOfSquares() + "\tFullModel df: " + fullModel.getDegreesOfFreedom() + "\tctModel df: " + ctModel.getDegreesOfFreedom());
//			double[] estimates1 = ctModel.getEstimateRegressionParameters();
//			double[] estimates0 = fullModel.getEstimateRegressionParameters();
//			System.out.println("Full model estimates");
//			for (int d = 0; d < estimates0.length; d++) {
//				System.out.println(d + "\t" + estimates0[d]);
//			}
//
//			System.out.println("CT model estimates:");
//			for (int d = 0; d < estimates1.length; d++) {
//				System.out.println(d + "\t" + estimates1[d]);
//			}
//			System.out.println("Pvalue: " + pval);
