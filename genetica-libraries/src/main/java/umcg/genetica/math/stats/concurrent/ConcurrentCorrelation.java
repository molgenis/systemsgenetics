...
public class ConcurrentCorrelation {
    ...
    public DoubleMatrix2D pairwiseCorrelation(DoubleMatrix2D in) {
        ExecutorService threadPool = Executors.newFixedThreadPool(nrThreads);
        CompletionService<Pair<Integer, double[]>> pool = new ExecutorCompletionService<Pair<Integer, double[]>>(threadPool);
        ...
        for (int i = 0; i < in.rows(); ++i) {
            ConcurrentCorrelationTask task = new ConcurrentCorrelationTask(in.toArray(), meanOfSamples, i);
            pool.submit(task);
        }

        int returned = 0;

        DenseDoubleMatrix2D correlationMatrix = new DenseDoubleMatrix2D(in.rows(), in.rows());
        ProgressBar pb = new ProgressBar(in.rows(), "Calculation of correlation matrix: " + in.rows() + " x " + in.rows());
