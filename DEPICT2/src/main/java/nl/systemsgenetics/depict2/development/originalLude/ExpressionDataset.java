...
package nl.systemsgenetics.depict2.development.originalLude;
        ...
        try {
            ...
            while ((str = in.readLine()) != null) {
                ...
                if (hashProbesToInclude==null || hashProbesToInclude.containsKey(data[0])) {
                    ...
                    for (int s=0; s<nrSamples; s++) {
                        if (data[sampleIndex[s] + sampleOffset].equals("NA")) {
                            rawData[nrProbes][s] = Double.NaN;
                        } else {
        ...
        for (int p=0; p<newProbeOrder.length; p++) {
            if (hashProbes.containsKey(newProbeOrder[p])) {
                if (!hashProbeIndex.containsKey(newProbeOrder[p])) {
                    hashProbeIndex.put(newProbeOrder[p], hashProbeIndex.size());
                } else {
                    System.out.println("Probe:\t" + newProbeOrder[p] + "\tIs present multiple times in new probe order!");
                    return;
                }
            } else {
                System.out.println("Probe:\t" + newProbeOrder[p] + "\tIs not present in this dataset!");
                return;
            }
        }
        if (hashProbeIndex.size()!=hashProbes.size()) {
            System.out.println("Number of probes is different!");
            return;
        }
        ExpressionDataset dataset = new ExpressionDataset(this.nrProbes, this.nrSamples);
        for (int s=0; s<this.nrSamples; s++) {
            dataset.sampleNames[s] = new String(this.sampleNames[s]);
        }
        for (int p=0; p<this.nrProbes; p++) {
            int newIndex = ((Integer) hashProbeIndex.get(this.probeNames[p])).intValue();
            dataset.probeNames[newIndex] = new String(this.probeNames[p]);
            for (int s=0; s<this.nrSamples; s++) {
                dataset.rawData[newIndex][s] = this.rawData[p][s];
            }
        }
        this.rawData = dataset.rawData;
        this.probeNames = dataset.probeNames;
        this.recalculateHashMaps();
    }
