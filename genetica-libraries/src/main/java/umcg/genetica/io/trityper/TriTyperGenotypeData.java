...
public class TriTyperGenotypeData {
    ...
    public final void load(String loc, String snpmaploc, String snploc) throws IOException {
        ...
        if (alInd.isEmpty()) {
            System.err.println(loc + ":\tERROR: your dataset does not contain any individuals.");
            System.exit(-1);
        }
...
//					}
        ...
        if (nrcasecontrolstatusunparseable > 0) {
            System.out.println(loc + ":\tWarning: case/control status unparseable for " + nrcasecontrolstatusunparseable + " individuals.");
        }
        if (nrexcludestatusunparseable > 0) {
            System.out.println(loc + ":\tWarning: include/exclude status unparseable for " + nrexcludestatusunparseable + " individuals.");
        }
        if (nrgenderstatusunparseable > 0) {
            System.out.println(loc + ":\tWarning: male/female status unparseable for " + nrgenderstatusunparseable + " individuals.");
        }

        System.out.println(loc + ":\t" + numInds + " individuals detected, " + numMales + " males, " + numFemales + " females, " + numCases + " cases, " + numControls + " controls, " + numIncluded + " included");

        if (numIncluded == 0) {
            System.err.println(loc + ":\tERROR: None of the samples in your dataset will be included. Please check your PhenotypeInformation.txt");
            System.exit(-1);
        }
        ...
        if (snploc != null) {
            ...
            if (Gpio.exists(loc + "SNPs.txt")) {
                ...
                throw new FileNotFoundException(loc + ":\tSNPs file not found");
            }
        }
        ...
        while (elems != null) {
            ...
            ctr++;
            if (ctr % 1000000 == 0) {
                System.out.print(t.getFileName() + ":\t" + ctr + " snps read\r");
            }
        }
        System.out.println();
        t.close();
        ...
        for (String s : tmpSNP) {
            if (snpToSNPId.containsKey(s)) {
                System.err.println(t.getFileName() + ":\tWarning: duplicate snp " + s);
            }
        ...
        System.out.println(loc + ":\t" + SNPs.length + " snps loaded");

        if (SNPs.length == 0) {
            System.err.println(loc + ":\tERROR: no SNPs have been detected. Please check your dataset.");
            System.exit(-1);
        }
        ...
        if (snpmaploc != null) {
            ...
            if (Gpio.exists(loc + "SNPMappings.txt")) {
                ...
                throw new FileNotFoundException(loc + ":\tSNPMappings not found");
            }
        }


        System.out.println(loc + ":\tReading: " + t.getFileName());
        lineElems = t.readLineElemsReturnReference(TextFile.tab);
        ...
        while (lineElems != null) {
            ...
            linenr++;
            if (linenr % 1000000 == 0) {
                System.out.print(t.getFileName() + ":\t" + linenr + " snp mappings read\r");
            }
            lineElems = t.readLineElemsReturnReference(TextFile.tab);
        ...
        t.close();
        System.out.println();
        System.out.println(loc + ":\tNr of SNPs without annotation: " + nrWoAnnotation + " / " + snpToSNPId.size());
