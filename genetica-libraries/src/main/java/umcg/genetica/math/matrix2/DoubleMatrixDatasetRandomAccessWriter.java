...
public class DoubleMatrixDatasetRandomAccessWriter {
	...
	public void open(String loc) throws IOException {
		...
		if (new File(loc + ".rows.txt").exists()) {
			hashRows = (LinkedHashMap<String, Integer>) DoubleMatrixDataset.loadIdentifiers(loc + ".rows.txt");
		} else if (new File(loc + ".rows.txt.gz").exists()) {
			hashRows = (LinkedHashMap<String, Integer>) DoubleMatrixDataset.loadIdentifiers(loc + ".rows.txt.gz");
		} else {
			throw new FileNotFoundException("File not found: " + loc + ".rows.txt or " + loc + ".rows.txt.gz");
		}

		if (new File(loc + ".cols.txt").exists()) {
			hashCols = (LinkedHashMap<String, Integer>) DoubleMatrixDataset.loadIdentifiers(loc + ".cols.txt");
		} else if (new File(loc + ".cols.txt.gz").exists()) {
			hashCols = (LinkedHashMap<String, Integer>) DoubleMatrixDataset.loadIdentifiers(loc + ".cols.txt.gz");
		} else {
			throw new FileNotFoundException("File not found: " + loc + ".cols.txt or " + loc + ".cols.txt.gz");
		}
