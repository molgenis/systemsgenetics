...
public class BinaryMetaAnalysisCLI {
	...
	public BinaryMetaAnalysisCLI(String[] args) {
		...
		try {
			...
			if (cmd.hasOption("calculateeqtlvariance")) {
			...
			} else if (cmd.hasOption("analyseindependentdatasets")) {
				...
				if (settings == null) {
					...
					IndependentDatasetAnalyzer bm = new IndependentDatasetAnalyzer(settings, texttoreplace, replacewith, usetmp);
