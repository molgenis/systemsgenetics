...
read.coreg.data.v2   <- function(path, phenotype, nrows=100, ncols=10, add.edges.for.cols=T, add.edges.for.rows=F) {
  ...
  coreg.tmp       <- read.depict2(paste0(path, "/", list.files(path, pattern="*.xlsx")))$Coregulation
...
depict.to.network.v2 <- function(cur.data, edge.threshold=2, ngwas=100, gene.name.thresh=3) {
  ...
  nodes$annot[nodes[,1] %in% cur.data$colgenes]                     <- "Mendelian gene" 
  # Is a bonf sig gwas gene?
  nodes$annot[nodes$annot == "Mendelian gene" & nodes[,1] %in% names(cur.data$gene.pvalues)[cur.data$gene.pvalues < (0.05/ length(cur.data$gene.pvalues))] ] <- "Both"
...
network  <- depict.to.network.v2(cur.data, edge.threshold=1, ngwas=280, gene.name.thresh = 10000)

#Make plots
pdf(width=15, height=12, useDingbats = F, file=paste0("~/Desktop/depict2/plots/", phenotype, "_edges_", edge.threshold, "_network_plot.pdf"))
par(xpd=NA)
...
# Ugly hack
pdf(width=8, height=5, useDingbats = F, file=paste0("~/Desktop/depict2/plots/ibd_core_genes.network.pdf"))
sub_mygraph <- to_subgraph(network$graph, gene_id %in% network$nodes[unique(c(network$edges[,1], network$edges[,2])), 1], subset_by = "nodes")$subgraph
...
# For patrick
write.table(network$nodes,file="~/Desktop/depict2/ibd_gastritis_nodes_for_patrick.tsv", sep="\t", row.names=F, quote=F)
...
write.table(tmp, file="~/Desktop/depict2/ibd_gastritis_edges_for_patrick.tsv", sep="\t", quote=F, row.names=F)
