 ...
 * @author lude
 */
public class ExpressionDatasetBool {

	public boolean[][] rawData = null;
	...
	public ExpressionDatasetBool(String fileName) {
	...
	public ExpressionDatasetBool(String fileName, String delimiter) {
		if (fileName.endsWith(".binary")) {
			loadExpressionDataInBinaryFormat(fileName);
		} else {
	...
	public ExpressionDatasetBool(String fileName, String delimiter, HashMap hashProbesToInclude) {
	...
	public ExpressionDatasetBool(String fileName, String delimiter, HashMap hashProbesToInclude, HashMap hashSamplesToInclude) {
		this.hashProbesToInclude = hashProbesToInclude;
	...
	public ExpressionDatasetBool(int nrProbes, int nrSamples) {
		...
		rawData = new boolean[nrProbes][nrSamples];
	}
		...
		try {
			...
			while ((str = in.readLine()) != null) {
				...
				if (hashProbesToInclude == null || hashProbesToInclude.containsKey(data[0])) {
					...
					for (int s = 0; s < nrSamples; s++) {
						switch (data[sampleIndex[s] + sampleOffset]) {
							case "0":
								rawData[nrProbes][s] = false;
								break;
							case "0.0":
								rawData[nrProbes][s] = false;
								break;
							default:
								rawData[nrProbes][s] = true;
						}
					}
					nrProbes++;
	...
	public boolean[][] getRawDataTransposed() {
		boolean[][] rawDataTransposed = new boolean[nrSamples][nrProbes];
		for (int s = 0; s < nrSamples; s++) {
			for (int p = 0; p < nrProbes; p++) {
