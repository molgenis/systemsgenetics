...
public class QQPlot {
    ...
    public void draw(String fileName, double fdrcutoff, int nrPermutationsFDR, int maxNrMostSignificantEQTLs, double[] uniquePermutedPvalues, double[] uniquePermutedPvaluesCounts, int nrPValues, TDoubleArrayList pValueRealData, ArrayList<Boolean> significantPvalue, int nrSignificantEQTLs) {
        System.setProperty("java.awt.headless", "true");
        ...
        double log10Max = -Math.log10(uniquePermutedPvalues[uniquePermutedPvalues.length-1]);
        double log10Max_r = -Math.log10(pValueRealData.get(pValueRealData.size()-1));
        if(log10Max_r>log10Max){
            log10Max = log10Max_r;
        }
