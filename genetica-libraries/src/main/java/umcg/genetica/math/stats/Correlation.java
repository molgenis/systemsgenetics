...
//    }
	...
	public static Pair<Double, Integer> correlateWithNaN(double[] x, double[] y) {
		if (x.length == y.length) {
			...
			int len = 0;
			for (int a = 0; a < x.length; a++) {
				if (!Double.isNaN(x[a]) && !Double.isNaN(y[a])) {
					double varXT = (x[a] - meanX);
					double varYT = (y[a] - meanY);
					covarianceInterim += varXT * varYT;
					varX += varXT * varXT;
			...
			double denominator = Math.sqrt(varX * varY);
			double covariance = covarianceInterim / (len - 1);
			double correlation = covariance / denominator;
			return new Pair<>(correlation, len);
		} else {
			System.out.println("Warning two arrays of non identical length are put in for correlation.");
			System.out.println("Returning NaN");
			return new Pair<>(Double.NaN, 0);
		}
	...
	public static double correlate(DoubleMatrix1D x, DoubleMatrix1D y) {
		if (x.size() == y.size()) {
			Pair<Double, Double> tmpMean = Descriptives.mean(x, y);
			double meanX = tmpMean.getLeft();
			double meanY = tmpMean.getRight();
			...
			for (int a = 0; a < x.size(); a++) {
				double varXT = (x.getQuick(a) - meanX);
				double varYT = (y.getQuick(a) - meanY);
				covarianceInterim += varXT * varYT;
			}

			double covariance = covarianceInterim / (x.size() - 1);
			return covariance;
		} else {
			System.out.println("Warning two arrays of non identical length are put in for correlation.");
			System.out.println("Returning NaN");
