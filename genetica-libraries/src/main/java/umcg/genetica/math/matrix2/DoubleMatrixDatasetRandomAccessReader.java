...
public class DoubleMatrixDatasetRandomAccessReader<R extends Comparable, C extends Comparable> {
	...
	public DoubleMatrixDatasetRandomAccessReader(String filename) throws IOException {
		...
		if (new File(filename + ".rows.txt").exists()) {
			...
			hashRows = (LinkedHashMap<R, Integer>) DoubleMatrixDataset.loadIdentifiers(filename + ".rows.txt.gz");
		} else {
		...
		if (new File(filename + ".cols.txt").exists()) {
			...
			hashCols = (LinkedHashMap<C, Integer>) DoubleMatrixDataset.loadIdentifiers(filename + ".cols.txt.gz");
		} else {
