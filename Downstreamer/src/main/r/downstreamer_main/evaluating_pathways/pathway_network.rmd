...
pathway.db <- "GO_BP"
datasets <- read.downstreamer.batch("../data/final_paper", USE.CACHE=F, potential_traits = c(pathway.db))
...
write.table(nodes, quote=F, row.names=F, sep="\t", file=paste0("output/plots/pathway_networks/", pathway.db, "_bonfsig_network.nodes.tsv"))

write.table(edges, quote=F, row.names=F, sep="\t", file=paste0("output/plots/pathway_networks/", pathway.db, "_bonfsig_network.edges.tsv"))
```
...
pdf(width=10, height=5, file=paste0("output/plots/pathway_networks/", pathway.db, "_bonfsig_network.pdf"), paper="a4")
p2
