...
public class AseMleNegative {
	...
	public AseMleNegative(IntArrayList a1Counts, IntArrayList a2Counts) {
		...
		for (int i = 0; i < a1Counts.size(); ++i) {
			...
			logBinominalCoefficients[i] = lnbico(totalReads - 1, a1Count);
		}
		...
		for (int i = 0; i < probabilities.length; ++i) {
			...
			double sumLogLikelihoodA = 0;
			double sumLogLikelihoodB = 0;
			double sumLogLikelihoodC = 0;
			double sumLogLikelihoodD = 0;
			double sumLogLikelihoodE = 0;
			...
			for (int s = 0; s < a1Counts.size(); ++s) {
				...
				//sumLogLikelihoodA = lnBeta((double) a1Counts.getQuick(s) + )
				
				int k = a1Counts.getQuick(s);
				int r = a2Counts.getQuick(s);
				
				sumLogLikelihoodA = gammln(k + r);
				sumLogLikelihoodB = factln(k);
				sumLogLikelihoodC = gammln(r);
				sumLogLikelihoodD = ((double) k) * logProbabilities[i];
				sumLogLikelihoodE = ((double) r) * log1minProbabilities[i];
				
				sumLogLikelihood += sumLogLikelihoodA - sumLogLikelihoodB - sumLogLikelihoodC + sumLogLikelihoodD + sumLogLikelihoodE;
