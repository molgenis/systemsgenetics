...
pdf(width=6, height=2, file="output/plots/gnomad_zscore_vs_max_zscore_degree_corrected.pdf", paper="a4")
grid.arrange(grobs=list(p1, p2, p3), ncol=3)
dev.off()
```
...
pdf(width=8, height=4, file= "output/plots/gnomad_pli_comparrison_all.pdf")
...
pdf(width=16, height=6, file= "output/plots/gnomad_zscore_comparrison_all.pdf")
grid.arrange(grobs=list(p1, p2, p3), ncol=3)
dev.off()
```
...
```{r}
zscores <- make.zscore.matrix(datasets)
write.table(zscores, file="output/key_gene_zscores_all_traits.tsv", sep="\t", quote=F)
...
write.table(tmp, file="output/key_gene_zscores_height_with_HP:0000925.tsv", sep="\t", quote=F)
