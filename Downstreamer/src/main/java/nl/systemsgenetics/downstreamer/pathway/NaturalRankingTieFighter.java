...
/**
 * HACKED class to support tie resolving for 1 specific use case!!!!!!
 ...
 * @since 2.0
 */
public class NaturalRankingTieFighter {
     ...
     */
    public NaturalRankingTieFighter() {
        super();
     ...
     */
    public NaturalRankingTieFighter(TiesStrategy tiesStrategy) {
        super();
     ...
     */
    public NaturalRankingTieFighter(NaNStrategy nanStrategy) {
        super();
     ...
     */
    public NaturalRankingTieFighter(NaNStrategy nanStrategy, TiesStrategy tiesStrategy) {
        super();
        this.nanStrategy = nanStrategy;
        this.tiesStrategy = tiesStrategy;
        randomData = new RandomDataGenerator();
     ...
     */
    public NaturalRankingTieFighter(RandomGenerator randomGenerator) {
        super();
    ...
    protected double[] rank(double[] data, double[] xWings) {
        ...
        for (int i = 0; i < data.length; i++) {
            ranks[i] = new IntDoubleDoubleTriple(data[i], i, xWings[i]);
        }
        ...
        for (int i = 1; i < ranks.length; i++) {
			int compareI = ranks[i].compareTo(ranks[i - 1]);
            if (compareI > 0) {
     ...
     */
    private IntDoubleDoubleTriple[] removeNaNs(IntDoubleDoubleTriple[] ranks) {
        if (!containsNaNs(ranks)) {
            return ranks;
        }
        IntDoubleDoubleTriple[] outRanks = new IntDoubleDoubleTriple[ranks.length];
        int j = 0;
        for (int i = 0; i < ranks.length; i++) {
            if (Double.isNaN(ranks[i].getValue())) {
                // drop, but adjust original ranks of later elements
                for (int k = i + 1; k < ranks.length; k++) {
                    ranks[k] =  new IntDoubleDoubleTriple(
                            ranks[k].getValue(), ranks[k].getPosition() - 1, ranks[k].getxWing());
                }
            } else {
                outRanks[j] = new IntDoubleDoubleTriple(
                        ranks[i].getValue(), ranks[i].getPosition(), ranks[i].getxWing());
                j++;
            }
        }
        IntDoubleDoubleTriple[] returnRanks = new IntDoubleDoubleTriple[j];
     ...
     */
    private void recodeNaNs(IntDoubleDoubleTriple[] ranks, double value) {
        for (int i = 0; i < ranks.length; i++) {
            if (Double.isNaN(ranks[i].getValue())) {
                ranks[i] = new IntDoubleDoubleTriple(
                        value, ranks[i].getPosition(), ranks[i].getxWing());
            }
        }
    }
     ...
     */
    private boolean containsNaNs(IntDoubleDoubleTriple[] ranks) {
        for (int i = 0; i < ranks.length; i++) {
            if (Double.isNaN(ranks[i].getValue())) {
                return true;
            }
     ...
     */
    private List<Integer> getNanPositions(IntDoubleDoubleTriple[] ranks) {
     ...
     */
    private static class IntDoubleDoubleTriple implements Comparable<IntDoubleDoubleTriple>  {
		...
		/** The max Z-score used to resolve the ties */
		private final double xWing;
         ...
         */
        IntDoubleDoubleTriple(double value, int position, double xWing) {
            this.value = value;
         ...
         */
        public int compareTo(IntDoubleDoubleTriple other) {
            int x = Double.compare(value, other.value); 
			
			if(x == 0){
				return Double.compare(xWing, other.xWing); 
			} else {
				return x;
			}
        }
