...
//				g2d.fillRect(xpos);
		...
		plotAxes(g2d, plotRange, nrPixelsMaxX, nrPixelsMaxY);
	...
	private void plotAxes(Graphics2D g2d, Range plotRange, int nrPixelsMaxX, int nrPixelsMaxY) {
		...
		if (dataRange.getMinX() < 0 && dataRange.getMaxX() > 0) {
			...
			for (double xval = 0; xval < plotRange.getMaxX() + (tickUnitX / 2); xval += tickUnitX) {
				...
				plotXAxisTick(xPosXAxis, xval, xPerc, nrPixelsMaxX, yPosXAxis, decimalFormat, g2d, metrics);
			}
			for (double xval = 0 - tickUnitX; xval > plotRange.getMinX(); xval -= tickUnitX) {
				double xPerc = plotRange.getRelativePositionX(xval);
				plotXAxisTick(xPosXAxis, xval, xPerc, nrPixelsMaxX, yPosXAxis, decimalFormat, g2d, metrics);
			}
		...
		if (dataRange.getMinY() < 0 && dataRange.getMaxY() > 0) {
			...
			for (double yval = 0 - tickUnitY; yval > plotRange.getMinY(); yval -= tickUnitY) {
				double yPerc = plotRange.getRelativePositionY(yval);
				plotYAxisTick(xPosYAxis, yval, yPerc, nrPixelsMaxY, decimalFormat, g2d, metrics, maxlen);
			}
	...
	private void plotYAxisTick(int xPosYAxis, double yval, double yPerc, int nrPixelsMaxY, DecimalFormat decimalFormat, Graphics2D g2d, FontMetrics metrics, int maxlen) {

		int ypos = y0 + marginY + (int) Math.ceil((1 - yPerc) * nrPixelsMaxY);
		int startx = xPosYAxis - 5;
		int stopx = startx + 5;
		g2d.drawLine(startx, ypos, stopx, ypos);
		String formattedStr = decimalFormat.format(yval);
	...
	private void plotXAxisTick(int xPosXAxis, double xval, double xPerc, int nrPixelsMaxX, int yPosXAxis, DecimalFormat decimalFormat, Graphics2D g2d, FontMetrics metrics) {

		int xpos = xPosXAxis + (int) Math.ceil(xPerc * nrPixelsMaxX);
		int starty = yPosXAxis;
		int stopy = starty + 5;

		String formattedStr = decimalFormat.format(xval);
		if (formattedStr.equals("-0")) {
			formattedStr = "0";
		}
