...
path <- "~/Desktop/depict_gls_test/ibd/"
trait <- "Coregulation"
...
index <- 1
for (chr in 1:22) {
  tmp.mat.q <- data.frame(fread(paste0(path, trait, "_", chr, "_q_Enrichment_geneCor.txt"), data.table = F), row.names = 1)

  tmp.mat.p <- tryCatch({
          tmp <- as.data.frame(fread(paste0(path, trait, "_", chr, "_p_Enrichment_geneCor.txt"), data.table = F))
          rownames(tmp) <- tmp[,1]
          tmp <- tmp[,-1, drop=F]
          tmp
        },
...
results <- apply(coreg, 2, function(y){
  custom.gls(gwas, y, gene.cor.inv)
})
...
results.2 <- apply(coreg, 2, function(y){
...
actual <- custom.gls(gwas, coreg[,"randPheno8"], gene.cor.inv)
rando <- custom.gls(gwas, coreg[,"randPheno9"], gene.cor.inv)

# Linear model for reference
results.lm <- apply(coreg, 2, function(y){
...
# Linear model for reference
results.chol <- apply(coreg, 2, function(y){
  yi <- Ci %*% y
  summary(lm(yi ~ xi))$coefficients[2,]
})
