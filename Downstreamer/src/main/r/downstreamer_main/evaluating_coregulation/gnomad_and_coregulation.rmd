...
write.table(tmp, "output/adjusted/max_coregulation_zscore_and_gnomad_metrics.tsv", sep="\t", quote=F)
...
pdf(width=6, height=2, file="output/plots/adjusted/gnomad_zscore_vs_max_zscore.pdf", paper="a4")
...
pdf(width=6, height=2, file="output/plots/adjusted/gnomad_zscore_vs_max_zscore_degree_corrected.pdf", paper="a4")
grid.arrange(grobs=list(p1, p2, p3), ncol=3)
dev.off()
```
...
p2 <- make.gnomad.scatterplot("mis_z") + ylab("Gnomad MiS Z-score") + theme(legend.position = "none")
p3 <- make.gnomad.scatterplot("lof_z") + ylab("Gnomad LoF Z-score") + theme(legend.position = "none")
pdf(width=6, height=2, file="output/plots/adjusted/gnomad_zscore_vs_max_genep.pdf", paper="a4")
...
pdf(width=5, height=2.5,file="output/plots/adjusted/GenePrioritization_classes_of_keygenes_vs_constraint.pdf", paper="a4")
theme.nature(p1)
dev.off()
...
pdf(width=8, height=4, file= "output/plots/adjusted/gnomad_pli_comparrison_all.pdf")
...
pdf(width=16, height=6, file= "output/plots/adjusted/gnomad_zscore_comparrison_all.pdf")
grid.arrange(grobs=list(p1, p2, p3), ncol=3)
dev.off()
```
...
```{r}
zscores <- make.zscore.matrix(datasets)
write.table(zscores, file="output/adjusted/key_gene_zscores_all_traits.tsv", sep="\t", quote=F)
...
write.table(tmp, file="output/adjusted/key_gene_zscores_height_with_HP:0000925.tsv", sep="\t", quote=F)
