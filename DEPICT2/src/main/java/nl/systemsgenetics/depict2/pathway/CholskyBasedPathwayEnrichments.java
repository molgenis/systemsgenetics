 ...
 * @author patri
 */
public class CholskyBasedPathwayEnrichments {
	...
	public CholskyBasedPathwayEnrichments(final PathwayDatabase pathwayDatabase,
		...
		try (ProgressBar pb = new ProgressBar(pathwayDatabase.getName() + " enrichment analysis", metaGenesPerArm.size() + 6, ProgressBarStyle.ASCII)) {
			...
			for (DoubleMatrixDataset<String, String> b1Arm : b1PerArm) {
				b1.getMatrix().assign(b1Arm.getMatrix(), DoubleFunctions.plus);
			}

			for (DoubleMatrixDataset<String, String> b2Arm : b2PerArm) {
				b2.getMatrix().assign(b2Arm.getMatrix(), DoubleFunctions.plus);
			}

			for (DoubleMatrixDataset<String, String> b1Arm : b1NullPerArm) {
				b1NullGwas.getMatrix().assign(b1Arm.getMatrix(), DoubleFunctions.plus);
			}

			for (DoubleMatrixDataset<String, String> b2Arm : b2NullPerArm) {
				b2NullGwas.getMatrix().assign(b2Arm.getMatrix(), DoubleFunctions.plus);
			}
			...
			for (int traitI = 0; traitI < numberTraits; ++traitI) {
				...
				for (int pathwayI = 0; pathwayI < numberOfPathways; ++pathwayI) {
					...
					double seBeta = Math.sqrt(sigmaSqrt) /	Math.sqrt(b1Trait);
			...
			for (int traitI = 0; traitI < numberTraitsNull; ++traitI) {
				...
				for (int pathwayI = 0; pathwayI < numberOfPathways; ++pathwayI) {
					...
					// Determine analytical pvalues
					GenePathwayAssociationStatistic curStat = determineGlsAnalyticalPvalues(
							beta,
							b1Trait,
							fullInverseCorrelationMatrix.getMatrix(),
							geneZscoresNullGwasNullBetasPathwayMatched.getCol(traitI),
							genePathwayZscores.getCol(pathwayI));

					pValuesNull.setElementQuick(pathwayI, traitI, curStat.getAnalyticalPvalue());
