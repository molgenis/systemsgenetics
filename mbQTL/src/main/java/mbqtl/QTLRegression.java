...
//        System.out.println("Re-ranking data:\t");
		...
		for (int g = 0; g < expressionData.genes.length; g++) {
			...
			if (geneAnnotationObj != null) {
				...
				int chr = geneAnnotationObj.getChromosome().getNumber();
...
//		for (int g = 0; g < expressionData.genes.length; g++) {
		...
		for (int gid = 0; gid < genes.size(); gid++) {
			...
			if (snpToRemove != null) {
				...
				// define CIS window
				int pos = geneAnnotationObj.getStart(); // geneAnnotation.getStartPos(geneAnnotationId);
				String geneSymbol = geneAnnotationObj.getGeneSymbol(); // geneAnnotation.getSymbol(geneAnnotationId);
