...
class GenotypeHarmonizer {
	...
	public static void main(String... args) throws InterruptedException {
		...
		String variantFilterListFile = commandLine.getOptionValue("vf", null);
		...
		try {
			...
			System.err.println(new StringBuilder().append("Error parsing --mafFilter \"").append(commandLine.getOptionValue("mf")).append("\" is not a double").toString());
			System.exit(1);
			return;
		}
		float minCallRate;
		try {
			...
			System.err.println(new StringBuilder().append("Error parsing --callRateFilter \"").append(commandLine.getOptionValue("cf")).append("\" is not an int").toString());
			System.exit(1);
			return;
		}
		...
		if ((minCallRate != 0.0F) || (minMAF != 0.0F) || (minHwePvalue != 0.0D) || (variantFilterListFile != null) || (seqFilterIn != null)) {
			...
			if (variantFilterListFile != null) {
				try {
					...
					BufferedReader variantIdFilterReader = new BufferedReader(new FileReader(variantFilterListFile));
					String line;
					...
					System.err.println("Unable to find file with variants to filter on at: " + variantFilterListFile);
					LOGGER.fatal("Unable to find file with variants to filter on at: " + variantFilterListFile);
					System.exit(1);
				} catch (IOException e) {
					System.err.println("Error reading file with variants to filter on at: " + variantFilterListFile + " error: " + e.getMessage());
					LOGGER.fatal("Error reading file with variants to filter on at: " + variantFilterListFile, e);
					System.exit(1);
				}
			}
			...
			if (seqFilterIn != null) {
				LOGGER.info("Filterig input on chromosome: " + seqFilterIn);
		...
		if (sampleFilterListFile != null) {
			try {
				...
				System.err.println("Unable to find file with samples to filter on at: " + sampleFilterListFile);
				LOGGER.fatal("Unable to find file with samples to filter on at: " + sampleFilterListFile);
				System.exit(1);
			} catch (IOException e) {
				System.err.println("Error reading file with samples to filter on at: " + sampleFilterListFile + " error: " + e.getMessage());
				LOGGER.fatal("Error reading file with samples to filter on at: " + sampleFilterListFile, e);
				System.exit(1);
			}
		}
