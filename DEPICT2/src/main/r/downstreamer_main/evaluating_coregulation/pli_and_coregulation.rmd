...
# HPO genes
hpo.genes <- read.table(hpo.file, stringsAsFactors = F, sep="\t")
hpo.genes <- gsub("\\.\\d+", "", ensembl[unique(hpo.genes$V2), "Gene.stable.ID.version"])

# Coregulation
files    <- list.files(main.downstreamer.output.path, pattern="*.xlsx", full.names = T)
datasets <- list()
for (file in files) {
...
pdf(width=10, height=4, file="output/plots/gnomad_lof_vs_bonf_sig.pdf")
p <- ggplot(df.plot, aes(y=pli, x=annot, fill=annot)) +
...
pdf(width=5, height=4, file="output/plots/gnomad_lof_vs_max_zscore.pdf")
