...
public class SimpleGeneticRiskScoreCalculator implements GeneticRiskScoreCalculator {
    ...
    //@Override
    public TObjectDoubleHashMap<String> calculateRiskScoresAlleleCounting(RandomAccessGenotypeData genotypeData) {
        int index;
        int useDosages = 0;
        ...
        // loop over risk snps
        for (index = 0; index < chr.size()-1; index++) {
            ...
            try {
                ...
                if (useDosages == 1) {
                    ...
                    for (index2 = 0; index2 < alleles.size(); index2++) {
                        if (index2 < 20) {
                            System.out.print(" -- allele " + alleles.get(index2).getAllelesAsString());
