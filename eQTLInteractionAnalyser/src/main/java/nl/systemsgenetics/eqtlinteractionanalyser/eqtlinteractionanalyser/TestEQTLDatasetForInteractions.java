...
public class TestEQTLDatasetForInteractions {
        ...
        try {
            ...
            while ((str = in.readLine()) != null) {
                if (!str.contains("NA")) {
                    ...
                    itr++;
                    if (itr%100==0) System.out.println(itr);
                }
            }                        
        } catch (Exception e) {
    ...
    public void performInteractionAnalysis(String[] covsToCorrect) {
        ...
        if (1==1) {
            ...
            if (1==1) {
                ...
                for (int s=0; s<datasetCovariates.nrSamples; s++) {
                    ...
                    if (Math.abs(z)>3) hashSamplesToExclude.put(datasetCovariates.sampleNames[s], null);
                }
            }
            if (1==1) {
        ...
        if (1==1) {
            ...
            ExpressionDataset datasetCovariatesToCorrectFor = new ExpressionDataset(cohorts.length + nrCompsToCorrectFor, datasetGenotypes.nrSamples);
            ...
            if (nrCompsToCorrectFor>0) {
                for (int comp=0; comp<nrCompsToCorrectFor; comp++) {
                    for (int s=0; s<datasetGenotypes.nrSamples; s++) {
                        datasetCovariatesToCorrectFor.rawData[4 + comp][s] = datasetCovariates.rawData[datasetCovariates.nrProbes - 51 + comp][s];
                    }
                }
            }
        ...
        if (1==1) {
            ...
            if (1==1) {
                ...
                String[] cohorts = {"LLDeep","LLS","RS","CODAM"};
                ExpressionDataset datasetCovariatesToCorrectFor = new ExpressionDataset(cohorts.length + covsToCorrect.length, datasetGenotypes.nrSamples);
                datasetCovariatesToCorrectFor.sampleNames = datasetGenotypes.sampleNames;
                for (int p=0; p<cohorts.length; p++) {
                    for (int s=0; s<datasetGenotypes.nrSamples; s++) {
                        if (datasetGenotypes.sampleNames[s].startsWith(cohorts[p])) {
                ...
                for (int c=0; c<covsToCorrect.length; c++) {
                    ...
                    for (int s=0; s<datasetGenotypes.nrSamples; s++) {
                        datasetCovariatesToCorrectFor.rawData[cohorts.length + c][s] = datasetCovariates.rawData[covsToCorrectIndex[c]][s];
                    }
                }
    ...
    public void makeInteractionPlot(String fileName, double[] genotype, double[] expression, double[] covariate, String[] sampleNames) {
        ...
        if (pValueInteraction > 0.001) pValueString = (new java.text.DecimalFormat("##.###;-##.###", new java.text.DecimalFormatSymbols(java.util.Locale.US))).format(pValueInteraction);
    ...
    public ExpressionDataset orthogonalizeMatrix(ExpressionDataset dataset) {
        ...
        for (int e=0; e<eigenValues.length; e++) {
            ...
            if (eigenValues[e]>1e-10) nrCompsWithPositiveEigenvalues++;
        }
